...
class CoalBlendOptimizer:
    def __init__(self, master):
        ...
        self.parsed_reflect_regions = self._parse_reflect_regions()
        self._setup_style()
        self.build_ui()
    ...
    def _build_constraints(self, quantity, df=None, main_volatile=None, max_diff=None):
        ...
        for col in ['ash', 'sulfur', 'reflect_std_max']:
            if col == 'reflect_std_max':
                constraints.append({
                    'type': 'ineq',
                    'fun': (lambda x, d=df, col=col: self.constraints[col] - self._calculate_reflect_std(x, d))
                })
            else:
                constraints.append({
                    'type': 'ineq',
                    'fun': (lambda x, c=col, d=df: (np.dot(x, d[c]) / x.sum()) - self.constraints[c][0])
                })
                constraints.append({
                    'type': 'ineq',
                    'fun': (lambda x, c=col, d=df: self.constraints[c][1] - (np.dot(x, d[c]) / x.sum()))
                })
        # 挥发分约束（动态）
        if main_volatile is not None and max_diff is not None:
            ...
            vmin, vmax = self.constraints['volatile']
        constraints.append({
            'type': 'ineq',
        ...
        for prop in ['G_value', 'Y_value', 'CSR']:
            constraints.append({
                'type': 'ineq',
                'fun': (lambda x, p=prop, d=df: self._calculate_property(x, p, d) - self.constraints[p][0])
            })
            constraints.append({
                'type': 'ineq',
                'fun': (lambda x, p=prop, d=df: self.constraints[p][1] - self._calculate_property(x, p, d))
            })
        constraints.append({
            'type': 'eq',
            'fun': (lambda x: x.sum() - quantity)
        })
        # 多煤种占比约束
        ...
        if hasattr(self, 'parsed_individual_perc_constraints') and self.parsed_individual_perc_constraints:
            for constraint_info in self.parsed_individual_perc_constraints:
                current_perc_constrained_indices = constraint_info['indices_in_df']
                min_perc_limit = constraint_info['min_perc']
                max_perc_limit = constraint_info['max_perc']
    ...
    def _calculate_property(self, ratios, prop_type, df=None):
        ...
        if col_name == 'reflect_std_max':
            return self._calculate_reflect_std(ratios, df)
    ...
    def _calculate_reflect_std(self, ratios, df):
        ...
        if total < 1e-9:
            return 0.0
        regions = self.parsed_reflect_regions
        valid_cols = []
        mid_points = []
        ...
        mixture_dist = mixture_dist / 100  # 转为比例
    ...
    def run_optimization(self):
        ...
        try:
            ...
            for idx, (i, main_coal) in enumerate(self.df.iterrows()):
                ...
                # 递归剔除法优化，传递主煤名
                res = find_best_volatile_solution(sub_df, quantity, self.constraints, self, max_diff, depth=1, main_coal_name=main_coal['coal_name'], main_volatile=main_volatile)
                if res:
    ...
    def _calculate_constraint_violation(self, solution, target_total, df=None):
        ...
        # 质量指标约束
        for prop in ['ash', 'sulfur', 'volatile', 'G_value', 'Y_value', 'CSR']:
            value = self._calculate_property(solution, prop, df)
            lower, upper = self.constraints[prop]
            if value < lower:
                total_violation += (lower - value) * 100
            elif value > upper:
                total_violation += (value - upper) * 100
        # 反射率标准差
        reflect_std = self._calculate_reflect_std(solution, df)
        if reflect_std > self.constraints['reflect_std_max']:
            total_violation += (reflect_std - self.constraints['reflect_std_max']) * 50
        # 多煤种独立占比约束
        if hasattr(self, 'parsed_individual_perc_constraints') and self.parsed_individual_perc_constraints:
            for c in self.parsed_individual_perc_constraints:
                idxs = c['indices_in_df']
        ...
        if 'volatile_diff_max' in self.constraints:
            ...
            if used_idx:
                ...
                vmin, vmax = min(used_v), max(used_v)
                diff = vmax - vmin
                if diff > self.constraints['volatile_diff_max']:
                    total_violation += (diff - self.constraints['volatile_diff_max']) * 1000
...
def find_best_volatile_solution(df, quantity, constraints, optimizer, max_diff, depth=1, path=None, main_coal_name=None, main_volatile=None):
    import copy
    if path is None:
        path = []
    ...
    try:
        ...
        # 构建约束，参数合法性检查
        cons = optimizer._build_constraints(quantity, df, main_volatile=main_volatile, max_diff=max_diff)
        ...
        if df.iloc[real_idx_max]['coal_name'] != main_coal_name:
            ...
            res1 = find_best_volatile_solution(df2, quantity, constraints, optimizer, max_diff, depth=depth+1, path=copy.deepcopy(path)+[('max', df.iloc[real_idx_max]['coal_name'])], main_coal_name=main_coal_name, main_volatile=main_volatile)
            if res1:
                return res1
        res2 = None
        if df.iloc[real_idx_min]['coal_name'] != main_coal_name and real_idx_min != real_idx_max:
            ...
            res2 = find_best_volatile_solution(df2, quantity, constraints, optimizer, max_diff, depth=depth+1, path=copy.deepcopy(path)+[('min', df.iloc[real_idx_min]['coal_name'])], main_coal_name=main_coal_name, main_volatile=main_volatile)
            if res2:
                return res2
