...
class CoalBlendOptimizer:
    ...
    def _calc_plan_metrics(self, solution, plan, stock_left=None, df=None):
        if df is None:
            df = self.df
        return optimizer_core.calc_plan_metrics(
            solution,
            df,
            self.constraints,
    ...
    def _build_constraints(self, quantity, df=None, main_volatile=None, max_diff=None):
        ...
        return optimizer_core.build_constraints(
            ...
            df,
            self.constraints,
            parsed_individual_perc_constraints=getattr(self, 'parsed_individual_perc_constraints', None),
            main_volatile=main_volatile,
            max_diff=max_diff,
            parsed_reflect_regions=self.parsed_reflect_regions,
        )
    ...
    def run_optimization(self):
        ...
        try:
            ...
            if max_diff is None:
                ...
                return
            result = optimizer_core.run_group_optimization(
                self.df,
                quantity,
                self.constraints,
            ...
            if result is not None:
                best_solution, best_main_coal, best_df, best_metrics = result
                best_cost = float(np.dot(best_solution, best_df['price']))
...
def find_best_volatile_solution(df, quantity, constraints, optimizer, max_diff, depth=1, path=None, main_coal_name=None, main_volatile=None):
    return optimizer_core.find_best_volatile_solution(
        df,
        quantity,
        constraints,
        getattr(optimizer, 'parsed_individual_perc_constraints', None),
        getattr(optimizer, 'parsed_reflect_regions', optimizer_core.parse_reflect_regions()),
        max_diff,
        depth=depth,
        path=path,
        main_coal_name=main_coal_name,
        main_volatile=main_volatile,
    )
